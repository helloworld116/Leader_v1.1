...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        self.label = [[UILabel alloc] initWithFrame:CGRectMake(0, 90, 50, 50)];
...
- (void)showTopMessage:(NSString *)warningText
{
    ...
    if (!topV) {
        topV = [[TopWarningView alloc] initWithFrame:CGRectMake(0, 90, CGRectGetWidth(self.view.bounds), 40)];
        objc_setAssociatedObject(self, &TopWarningKey, topV, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    topV.frame = CGRectMake(0, 90, CGRectGetWidth(self.view.bounds), 40);
